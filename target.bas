...
220 A=RND(0)*TWOPI:B=RND(0)*TWOPI:Q=INT(A*R1):W=INT(B*R1)
260 PRINT "RADIANS FROM X AXIS =";A;"    FROM Z AXIS =";B
270 PRINT "APPROX DEGREES FROM X AXIS =";Q;"    FROM Z AXIS =";W
280 P=100000*RND(0)+RND(0):X=SIN(B)*COS(A)*P
290 Y=SIN(B)*SIN(A)*P:Z=COS(B)*P
340 PRINT "TARGET SIGHTED: APPROX COORDINATES X=";X;"   Y=";Y;
342 PRINT "  Z=";Z
...
400 PRINT "INPUT ANGLE DEVIATION FROM X, DEVIATION FROM Z,";
405 PRINT " DISTANCE";:INPUT A1,B1,P2
410 PRINT:IF P2>=20 THEN 420
415 PRINT "YOU BLEW YOURSELF UP!!":GOTO 580
420 A1=A1/R1:B1=B1/R1
422 PRINT "RADIANS FROM X AXIS =";A1;"    FROM Z AXIS =";B1
480 X1=P2*SIN(B1)*COS(A1)
482 Y1=P2*SIN(B1)*SIN(A1)
484 Z1=P2*COS(B1)
510 D=((X1-X)^2+(Y1-Y)^2+(Z1-Z)^2)^(1/2)
520 IF D>20 THEN 670
530 PRINT:PRINT " * * * HIT * * *   TARGET IS NON-FUNCTIONAL"
535 PRINT
550 PRINT "DISTANCE OF EXPLOSION FROM TARGET WAS";D;" KILOMETERS"
570 PRINT:PRINT "MISSION ACCOMPLISHED IN";R;" SHOTS."
580 R=0:FOR I=1 TO 5:PRINT:NEXT I
585 PRINT "NEXT TARGET....":PRINT
590 GOTO 220
670 X2=X1-X:Y2=Y1-Y:Z2=Z1-Z
675 IF X2<0 THEN 730
...
820 PRINT "APPROX POSITION OF EXPLOSION:  X=";X1;
822 PRINT "   Y=";Y1;"    Z=";Z1
830 PRINT "   DISTANCE FROM TARGET =";D
835 PRINT:PRINT:PRINT:GOTO 345
999 END
