...
410 IF F(A(K)-1,B(K))=0 THEN 420
412 IF F(A(K)-1,B(K))<>F(A(K),B(K)-1) THEN 450
420 A(K+1)=A(K)-1
430 B(K+1)=B(K)-1
440 GOTO 530
450 M=2
460 Z1=MAX(A(1),A(2),A(3))
470 Z2=MAX(B(1),B(2),B(3))
480 IF Z1=6 OR Z2=6 THEN 90
490 IF F(Z1+1,Z2+1)>0 THEN 90
500 IF F(Z1,Z2+1)=0 THEN 510
502 IF F(Z1,Z2+1)<>F(Z1+1,Z2) THEN 90
510 A(K+1)=Z1+1
520 B(K+1)=Z2+1
530 NEXT K
540 GOTO 950
...
820 IF F(A(K)+1,B(K))=0 THEN 830
822 IF F(A(K)+1,B(K))<>F(A(K),B(K)-1) THEN 860
830 A(K+1)=A(K)+1
840 B(K+1)=B(K)-1
850 GOTO 940
860 M=2
870 Z1=MIN(A(1),A(2),A(3))
880 Z2=MAX(B(1),B(2),B(3))
890 IF Z1=1 OR Z2=6 THEN 90
900 IF F(Z1-1,Z2+1)>0 THEN 90
910 IF F(Z1,Z2+1)=0 THEN 920
912 IF F(Z1,Z2+1)<>F(Z1-1,Z2) THEN 90
920 A(K+1)=Z1-1
930 B(K+1)=Z2+1
940 NEXT K
950 F(A,B)=9-2*I-J
...
1000 NEXT I
1010 PRINT
1020 PRINT "THE FOLLOWING CODE OF THE BAD GUYS' FLEET"
1022 PRINT "DISPOSITION HAS BEEN CAPTURED BUT NOT DE-CODED;"
1030 PRINT
1050 FOR I=1 TO 6
1052 FOR J=1 TO 6
1054 H(I,J)=F(J,I)
...
1300 PRINT "THERE USED TO BE A SHIP AT THAT POINT,"
1302 PRINT "BUT YOU SUNK IT."
